                ...
                <div class="col-12">
                    <div class="card card-primary">
                        <div class="card-header">
                            <div class="row">
                                <div class="col-9">
                                    <h3 class="card-title">Management Unit Kerja</h3>
                                </div>
                                <div class="col-3">
                                    <button type="button" class="btn btn-warning btn-block btn-flat" onclick="addData()"><i class="fa fa-plus"></i> Add Unit Kerja </button>
                                </div>
                            </div>
                        </div>
                        <!-- /.card-header -->
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <form action="#" id="form">
                    <!-- primary key -->
                    <input type="hidden" name="unitkerja_id">

                    <div class="form-group">
                        <div class="row align-items-center">
                        <label class="col-sm-2">Unit Kerja :</label>
                        <div class="col-sm-10">
                            <input type="text" class="form-control" name="unitkerja" placeholder="nama unitkerja">
                        </div>
                        </div>
                    </div> 
                </form>
...
<script type="text/javascript">
    ...
    function addData(){
        resetForm();
        $(".modal-title").html('Add Data Unit Kerja');
    ...
    function save(){
        ...
        var id = $('[name="unitkerja_id"]').val();
        var url = id ==  '' ? "<?=base_url().'spadmin/unitkerja/add'?>" : "<?=base_url().'spadmin/unitkerja/update'?>";
    ...
    function edit(id){
        resetForm();
        $(".modal-title").html('Edit Data Unit Kerja');
        $.ajax({
            type : "GET",
            url  : "<?php echo base_url('spadmin/unitkerja/edit')?>/" + id,
            dataType : "JSON",
            success: function(data){
              $('#modal_add').modal('show');
              $('[name="unitkerja_id"]').val(data.unitkerja_id);
    ...
    function hapus(id){
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    type : "GET",
                    url  : "<?php echo base_url('spadmin/unitkerja/hapus')?>/" + id,
                    dataType : "JSON",
                    success: function(data){
                        if(data.data){
                            reload_table();
